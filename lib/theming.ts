...
const createTheming = function createTheming<Theme>(
  ...
  const ThemeProvider = defineComponent({
    ...
    setup(props, { slots }) {
      const outerTheme: Ref<Theme> | undefined = inject(contextKey)
      ...
      const theme = computed(() => {
        ...
        if (typeof pt === 'function') {
          ...
          theme =
            outerTheme && outerTheme!.value ? { ...(outerTheme!.value as any), ...(pt as any) } : pt
          console.warn(isObject(theme), '[ThemeProvider] Please make your theme prop a plain object')
        }
